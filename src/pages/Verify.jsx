...
const Verify = () => {
  ...
  // Delivery Option & Address State
  const [deliveryOption, setDeliveryOption] = useState('digital');
  const [address, setAddress] = useState('');

  const certificateRef = useRef();

  // DYNAMIC PRICING LOGIC (Digital is now FREE, only printed has charges)
  const getPrices = (durationStr = "1 Month") => {
    if (durationStr.includes("3")) return { digital: 0, printed: 450 };
    if (durationStr.includes("6")) return { digital: 0, printed: 700 };
    return { digital: 0, printed: 299 }; // Default 1 Month
  };
  ...
  const handlePayment = async () => {
    try {
      // AGAR DIGITAL HAI TOH BINA PAYMENT KE DIRECT CERTIFICATE CLAIM KAR LO
      if (deliveryOption === 'digital') {
        ...
        if (verifyData.success) {
          ...
          setPaymentSuccess(true);
          alert("Free Digital Certificate Unlocked Successfully! 🎉");
        } else {
      ...
      // AGAR PRINTED HAI TOH ADDRESS CHECK KARO AUR RAZORPAY KHOLO
      if (!address.trim()) {
      ...
      const options = {
        ...
        description: 'Printed + Courier Certificate',
        ...
        handler: async function (response) {
          ...
          if (verifyData.success) {
            ...
            setIssueDate(verifyData.issueDate);
            setPaymentSuccess(true);
            alert("Payment Successful! Aapka Printed Certificate order placed ho gaya hai.");
          } else {
                  ...
                  <div className="space-y-6">
                    {!paymentSuccess ? (
                      <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <label className={`cursor-pointer border-2 rounded-xl p-4 flex flex-col items-center justify-center gap-2 transition-all ${deliveryOption === 'digital' ? 'border-blue-600 bg-blue-50' : 'border-slate-200 hover:border-blue-300'}`}>
                            ...
                            <div className="text-center">
                              <p className="font-bold text-slate-800">Digital Certificate</p>
                        ...
                        {/* Address Input Box (Sirf Printed select hone par dikhega) */}
                        ...
                        <button onClick={handlePayment} className={`w-full font-bold py-3.5 px-4 rounded-lg flex items-center justify-center gap-2 transition-colors shadow-md text-lg ${deliveryOption === 'digital' ? 'bg-green-600 hover:bg-green-700 text-white' : 'bg-slate-900 hover:bg-black text-white'}`}>
                          {deliveryOption === 'digital' ? <><Download size={20} /> Claim Free Digital Certificate</> : <><CreditCard size={20} /> Pay ₹{getPrices(result.user.duration).printed} for Printed Certificate</>}
                        </button>
                      </>
                    ) : (
